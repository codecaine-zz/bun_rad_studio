...
usage() {
  ...
  echo
  echo "Installs the required Homebrew dependencies for Bun RAD Studio."
...
if [[ "$INCLUDE_OPTIONAL" -eq 1 ]]; then
  echo "Installing required Bun RAD Studio dependencies plus optional cross-compilation toolchains..."
else
  echo "Installing required Bun RAD Studio dependencies with Homebrew..."
fi
